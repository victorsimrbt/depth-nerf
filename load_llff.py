...
def _minify(basedir, factors=[], resolutions=[]):
    needtoload = False
    for r in factors:
        imgdir = os.path.join(basedir, 'images_{}'.format(r)).replace("\\","/")
        if not os.path.exists(imgdir):
            needtoload = True
    for r in resolutions:
        imgdir = os.path.join(basedir, 'images_{}x{}'.format(r[1], r[0])).replace("\\","/")
        if not os.path.exists(imgdir):
            needtoload = True
    if not needtoload:
        return
    ...
    imgdir = os.path.join(basedir, 'images').replace("\\","/")
...
def load_colmap_depth(basedir, factor=8, bd_factor=.75):
    ...
    for id_im in range(1, len(images)+1):
        ...
        if len(depth_list) > 0:
            ...
            data_list.append({"depth":np.array(depth_list), "coord":np.array(coord_list), "weight":np.array(weight_list), "error":np.array(error_list)})
        else:
